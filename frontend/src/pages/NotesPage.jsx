...
class ErrorBoundary extends React.Component {
  ...
  render() {
    if (this.state.hasError) {
      return (
        <div className="flex flex-col items-center justify-center w-full h-screen bg-[#0a0a0a] text-red-500 p-10 font-mono">
          <h1 className="text-3xl font-bold mb-4">React Render Crash Protected</h1>
          <div className="p-6 bg-red-500/10 border border-red-500/30 rounded-xl w-full max-w-3xl overflow-auto text-left">
            <span className="font-bold text-red-400">Error:</span> {this.state.error?.message}
          </div>
          <button onClick={() => window.location.reload()} className="mt-8 px-6 py-2 bg-white/10 hover:bg-white/20 text-white rounded-lg transition-colors">Reload Page</button>
        </div>
      );
    }
...
const useNotes = () => {
  ...
  const saveNote = async (data) => {
    try {
      ...
      return true;
    } catch (err) { alert("❌ Network Error: Backend is down!"); return false;}
  };
              ...
              <div className="w-full max-w-5xl h-[90vh] bg-[#0a0a0a]/40 backdrop-blur-3xl border border-white/20 rounded-3xl shadow-[0_8px_32px_rgba(0,0,0,0.5)] flex flex-col overflow-hidden animate-in zoom-in-95">
                ...
                <div className="h-20 shrink-0 border-b border-white/10 px-8 flex items-center justify-between bg-white/5">
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <button onClick={() => setIsEditorOpen(false)} className="w-10 h-10 flex items-center justify-center rounded-xl bg-white/10 hover:bg-red-500/20 hover:text-red-400 text-white border border-white/10"><X className="w-5 h-5" /></button>
                  </div>
                </div>

                <div className="px-8 py-5 border-b border-white/10 flex items-center gap-6 bg-white/5 shrink-0">
                  <input type="text" autoComplete="off" value={editorData.title} onChange={(e) => setEditorData({...editorData, title: e.target.value})} placeholder="Note Title..." className="flex-1 bg-transparent text-3xl font-extrabold text-white placeholder-white/30 border-none outline-none drop-shadow-md" />
                    ...
                    <div className="max-w-4xl mx-auto flex flex-col gap-6">
                      <div className="p-4 rounded-2xl bg-black/20 border border-white/10 backdrop-blur-md">
                        <div className="flex flex-wrap gap-2 mb-3">
                          {safeEditorTags.map((tag, idx) => (<span key={idx} className="px-3 py-1 rounded-md text-xs font-bold bg-white/10 text-white flex items-center gap-2 border border-white/20 shadow-sm">{tag} <button onClick={() => setEditorData({...editorData, tags: editorData.tags.filter(t => t !== tag)})} className="hover:text-red-400"><X className="w-3 h-3"/></button></span>))}
                        </div>
                        <input type="text" autoComplete="off" value={tagInput} onChange={(e) => setTagInput(e.target.value)} onKeyDown={(e) => { if(e.key==='Enter') { e.preventDefault(); addTag(tagInput); } }} placeholder="Type a custom tag and press Enter..." className="w-full bg-transparent border-none outline-none text-sm text-white/90 placeholder-white/40" />
                      </div>
                      <textarea value={editorData.content} onChange={(e) => setEditorData({...editorData, content: e.target.value})} placeholder="Start writing your thoughts here..." className="w-full min-h-[500px] bg-transparent text-lg text-white font-medium placeholder-white/30 border-none outline-none resize-none leading-relaxed drop-shadow-sm" />
                    </div>
                  )}
                  ...
                  {editorData.editorMode === 'pdf' && (
                    <div className="w-full h-[600px] flex flex-col gap-4">
                      {!editorData.content || !editorData.content.startsWith('data:application/pdf') ? (
                        <div className="w-full h-full border-2 border-dashed border-white/30 rounded-2xl flex flex-col items-center justify-center text-white/60 bg-black/20 hover:bg-white/10 cursor-pointer relative backdrop-blur-sm transition-all">
                          <UploadCloud className="w-12 h-12 mb-4 text-white/80" />
                          <h3 className="text-xl font-bold text-white mb-2 drop-shadow-md">Upload PDF</h3>
                        ...
                        <div className="w-full h-full rounded-2xl overflow-hidden border border-white/20 shadow-2xl relative bg-white/5">
                          ...
                          <embed src={editorData.content} type="application/pdf" width="100%" height="100%" className="rounded-xl" />
                        </div>
                      )}
                    </div>
                  )}
            ...
            <div className="fixed inset-0 z-[1000] flex items-center justify-center bg-black/60 backdrop-blur-md animate-in fade-in">
              <div className="w-[400px] bg-[#0a0a0a]/80 backdrop-blur-3xl border border-white/20 rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in-95">
                ...
                <form onSubmit={(e) => { e.preventDefault(); createFolder(folderModal.name); setFolderModal({isOpen: false, name: ''}); }} className="p-6 flex flex-col gap-4">
                  ...
                  <button type="submit" disabled={!folderModal.name.trim()} className="w-full py-3 rounded-xl bg-indigo-500 hover:bg-indigo-600 disabled:opacity-50 text-white font-bold shadow-lg">Create</button>
                </form>
              </div>
            </div>
          )}
